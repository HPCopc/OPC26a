...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = Hub.listen('auth', ({ payload }) => {
      if (payload.event === 'signedIn')  { setIsLoggedIn(true);  checkUser(); }
      if (payload.event === 'signedOut') { setIsLoggedIn(false); setUserInfo(null); }
    });
  ...
  return (
    <header className="border-b border-black">
      ...
      {/* Utility bar */}
      <div className="bg-black text-white p-4 flex justify-between" role="navigation" aria-label="Utility">
        <ul className="flex flex-row gap-4 list-none text-sm">
      ...
      <div className="flex flex-row justify-between">
        <div className="flex flex-row items-center">
          ...
          <Link href="/"><img src="/opcdrop.png" alt="OPC logo" /></Link>
          ...
          <ul className="flex flex-row items-center gap-4">
            {primaryNav.filter(item => !item.children).map(item => (
              <li key={item.label}>
                <Link href={item.href || '#'}>{item.label}</Link>
              </li>
            ))}
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-1 w-48 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden">
                  ...
                  {userInfo?.role === 'ADMINS' && (
                    <Link href="/admin/pages" onClick={() => setDropdownOpen(false)}
                      className="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 border-b border-gray-100">
                      <svg className="w-4 h-4" viewBox="0 0 16 16" fill="none">
